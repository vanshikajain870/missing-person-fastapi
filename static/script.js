...
// ══════════════════════════════════════════════════════
// RENDER ADMIN TABLE  — fetches from FastAPI (app.py)
// ══════════════════════════════════════════════════════
async function renderAdminTable() {

    const searchValue  = document.getElementById("admin-srch") ? document.getElementById("admin-srch").value.toLowerCase() : "";
    const statusFilter = document.getElementById("admin-filt") ? document.getElementById("admin-filt").value : "";
    ...
    try {
        ...
        tb.innerHTML = filtered.map(r => {
            ...
            const statusClass = status.toLowerCase();
            const photoHtml   = r.photo_path
                ? `<img src="https://missing-person-fastapi.onrender.com/${r.photo_path}" width="38" height="38" style="border-radius:9px;object-fit:cover;">`
                : `<i class="fas fa-user"></i>`;
            const foundBtn = status !== "Found"
                ? `<button class="ar-btn ar-found-btn" onclick="markAdminFound('${r._id}')">✅ Found</button>`
                : "";
...
// ══════════════════════════════════════════════════════
// FORM SUBMISSIONS
// NOTE: submitAdminInmateReport  → owned by connect1.js
//       submitPublicLostReport   → owned by connect.js
// Do NOT define them here — that causes the old local
// version to override the API version.
// ══════════════════════════════════════════════════════
